...
def calculate_demographic_data(print_data=True):
    ...
    total_rows = len(df)
    percentage_bachelors = round(100 * len(df.loc[df['education'] == 'Bachelors'])/total_rows,1)
    ...
    higher_education = df.loc[df['education'].isin(higher_ed)]
    lower_education = df.loc[~df['education'].isin(higher_ed)]
    ...
    rich_percentage = round(100 * len(df.loc[(df['hours-per-week'] == min_work_hours) & (df['salary'] == '>50K')])/num_min_workers,1)

    # What country has the highest percentage of people that earn >50K?
    country_percentages = (100 * df.loc[df['salary'] == '>50K', 'native-country'].value_counts() / df['native-country'].value_counts()).sort_values(ascending=False)
